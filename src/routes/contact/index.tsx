   ...
   const handleFormSubmit = async (e: React.FormEvent) => {
      ...
      try {
         ...
         if (res.ok) {
            ...
            setFormData({ firstName: '', lastName: '', email: '', category: 'General Inquiry', message: '' });
         } else {
            Swal.fire({
               icon: 'error',
               title: 'Oops...',
               ...
               <div className="mx-auto max-w-7xl px-6 grid lg:grid-cols-[400px_1fr] gap-20 items-start">
                  ...
                  <div className="relative p-10 rounded-[3rem] bg-white border border-forest-deep/5 shadow-2xl">
                     <div className="mb-10 text-center">
                        <span className="inline-block px-4 py-1.5 rounded-full bg-forest-deep/5 text-forest-deep font-bold text-[10px] uppercase tracking-widest mb-4">Direct Message</span>
                        <h3 className="text-3xl font-display font-bold text-forest-deep mb-3">Drop us a Line</h3>
                        <p className="text-forest-deep/60 text-sm">Select a topic below so we can route your message to the right person.</p>
                     </div>

                     <form onSubmit={handleFormSubmit} className="space-y-8">
                        <div className="grid sm:grid-cols-2 gap-6">
                           <div className="space-y-2">
                              <label className="text-[10px] font-bold uppercase tracking-widest text-forest-deep/40 ml-1">First Name</label>
                              <input required type="text" value={formData.firstName} onChange={e => setFormData({ ...formData, firstName: e.target.value })} className="w-full bg-[#FAF7EE] border border-forest-deep/5 rounded-2xl px-6 py-4 text-forest-deep focus:outline-none focus:border-gold/50 transition-all" />
                           </div>
                           <div className="space-y-2">
                              <label className="text-[10px] font-bold uppercase tracking-widest text-forest-deep/40 ml-1">Last Name</label>
                              <input required type="text" value={formData.lastName} onChange={e => setFormData({ ...formData, lastName: e.target.value })} className="w-full bg-[#FAF7EE] border border-forest-deep/5 rounded-2xl px-6 py-4 text-forest-deep focus:outline-none focus:border-gold/50 transition-all" />
                           </div>
                        </div>

                        <div className="space-y-2">
                           <label className="text-[10px] font-bold uppercase tracking-widest text-forest-deep/40 ml-1">Email Address</label>
                           <input required type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full bg-[#FAF7EE] border border-forest-deep/5 rounded-2xl px-6 py-4 text-forest-deep focus:outline-none focus:border-gold/50 transition-all" />
                        </div>

                        <div className="space-y-2">
                           <label className="text-[10px] font-bold uppercase tracking-widest text-forest-deep/40 ml-1">Inquiry Type</label>
                           <select value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className="w-full bg-[#FAF7EE] border border-forest-deep/5 rounded-2xl px-6 py-4 text-forest-deep focus:outline-none focus:border-gold/50 transition-all appearance-none cursor-pointer">
                        ...
                        <div className="space-y-2">
                           ...
                           <textarea required rows={5} value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })} className="w-full bg-[#FAF7EE] border border-forest-deep/5 rounded-2xl px-6 py-4 text-forest-deep focus:outline-none focus:border-gold/50 transition-all resize-none"></textarea>
                        </div>

                        <button type="submit" disabled={isSubmitting} className="w-full py-5 rounded-2xl bg-forest-deep text-gold font-bold text-sm uppercase tracking-widest shadow-xl hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:scale-100 transition-all flex items-center justify-center gap-3">
